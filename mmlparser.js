...
function sepreateMMLtoNote(Data){
    ...
    Data.forEach((x)=>{
        ...
        if(regOption.test(x)){
            ...
            if(x.match("v")){if(x.match(regRange[1])){defaultTempo = x.match(regRange[1])}}
        }
    })
...
function MMLParser(Data){
    ...
    //Note 체크.
    if( /[abcdefgrotvln><]/.test(Data[cursor]) ){
    }
